...
| Сигналы и веса | Активное время в модалке `+0.2/сек` (cap 8 баллов за одно открытие); просмотр **уникального** фото `+1.5/фото` (cap 6 баллов за одно открытие; «уникальное» = URL, которого ещё не было в `photosViewed[id]` за всю историю просмотров этой модели — повторные просмотры одного и того же фото балл не дают); 2-е открытие модели `+3` разово; 3-е и далее `+5` за каждое; дошёл до последнего фото в галерее `+2` разово (за всю историю). |
...
(function () {
  ...
  //   при 'gallery:close' (юзер закрыл галерею не среагировав на toast) →
  //     InterestTracker.markDismissed(id), toast slide-out.
  //     Это даёт честный 7-дневный кулдаун: иначе игнорировавший toast увидит его
  //     снова при первом же открытии этой модели в новой сессии.
