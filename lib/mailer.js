...
function buildPayloadRows(payload, source) {
  ...
  return Object.keys(payload).map(function (key) {
    ...
    if (source === 'quiz') {
      ...
      if (t) return { label: t.label, value: t.value };
    }
    const label = FIELD_LABEL[key] || key;
    return { label: label, value: stringifyValue(raw) };
  });
}
...
const STYLE = {
  ...
  header:  'background:#0a3d62;color:#ffffff;padding:18px 22px;border-radius:8px 8px 0 0;',
...
function sectionTitleFor(source) {
  ...
  if (source === 'service') return 'Запрошенная услуга';
